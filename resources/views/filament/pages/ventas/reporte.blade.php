    ...
    <div class="space-y-6" x-data x-on:sales-results-ready.window="$nextTick(() => document.getElementById('sales-report')?.scrollIntoView({ behavior: 'smooth', block: 'start' }))">
        ...
        @if ($hasSearched)
            ...
            <div id="sales-report" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <x-filament::section compact class="opm-kpi-card" style="--opm-kpi-color: #d97706">
            ...
            <x-filament::section>
                ...
                @elseif (count($salesRows))
                    @include('filament.pages.ventas.partials.table', ['withDetail' => false])
                @else
                    <x-filament::empty-state icon="heroicon-o-chart-bar" heading="No hay ventas para los filtros seleccionados." />
                @endif
            </x-filament::section>
        @endif
    </div>
