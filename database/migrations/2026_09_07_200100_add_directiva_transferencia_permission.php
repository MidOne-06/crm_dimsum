...
return new class extends Migration
{
    ...
    {
        $permission = ['name' => 'Ver directiva de transferencia', 'slug' => 'directiva-transferencia.view', 'module' => 'Stock Inicial', 'is_system' => true, 'created_at' => now(), 'updated_at' => now()];
    ...
    {
        $id = DB::table('permissions')->where('slug', 'directiva-transferencia.view')->value('id');
        if ($id) DB::table('permission_role')->where('permission_id', $id)->delete();
        DB::table('permissions')->where('slug', 'directiva-transferencia.view')->delete();
    }
};
